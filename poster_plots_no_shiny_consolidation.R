...
results_agegroup <- lapply(ssa_iso3, function(x) {
  print(x)
  dir <- orderly::orderly_search(
    "latest(parameter:cntry == cntry)", 
    "02_aggregations", 
    parameters = list(cntry = x)
  )
  results_reader(
    dir_path = file.path("archive/02_aggregations", dir, "artefacts/"), 
    type = "age_group"
  )
})

results_agegroup <- rbindlist(results_agegroup)

# temp: load other KEN data 
results_agegroup <- results_agegroup %>% 
  filter(!grepl("KEN", area_id))

results_ken <- results_reader(
  dir_path = "../circumcision-coverage/Runs/aggregations/KEN_1953/data/", 
  type = "age_group"
)

results_agegroup <- bind_rows(results_agegroup, results_ken)
...
areas <- sf::read_sf("archive/00a2_areas_join/20220621-162124-d75bc170/artefacts/areas.geojson") %>% 
  dplyr::mutate(space = seq_len(dplyr::n()))
...
results_target <- results_target %>% 
  mutate(
    `MC Coverage` = ifelse(Country == "Kenya", 0.931, `MC Coverage`),
    `MMC Coverage` = ifelse(Country == "Kenya", 0.655, `MMC Coverage`),
    `TMC Coverage` = ifelse(Country == "Kenya", 0.276, `TMC Coverage`),
    lower_mc = ifelse(Country == "Kenya", 0.918, lower_mc),
    lower_mc = ifelse(Country == "Kenya", 0.636, lower_mmc),
    lower_mc = ifelse(Country == "Kenya", 0.266, lower_tmc),
    upper_mc = ifelse(Country == "Kenya", 0.942, upper_mc),
    upper_mmc = ifelse(Country == "Kenya", 0.670, upper_mmc),
    upper_tmc = ifelse(Country == "Kenya", 0.287, upper_tmc)
  )

# reactable table 
p4 <- reactable(
  # remove CI columns
  data = select(results_target, -c(contains("upper"), contains("lower"), matches("NA"))),
